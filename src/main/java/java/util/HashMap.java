...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     *
     * @param  initialCapacity the initial capacity 31
     * @param  loadFactor      the load factor 0.75f
    ...
    static final class TreeNode<K,V> extends LinkedHashMap.Entry<K,V> {
        ...
        static <K,V> TreeNode<K,V> balanceInsertion(TreeNode<K,V> root,
            ...
            //默认新结点为红色
            x.red = true;
            //xp表示x的父节点，xpp表示x的祖父结点，xppl表示xpp左孩子结点，xppr表示xpp的右结点
            for (TreeNode<K,V> xp, xpp, xppl, xppr;;) {
                //如果父节点为空的时候x，则表示x为第一个结点即根结点就变黑
                if ((xp = x.parent) == null) {
                    x.red = false;
                    return x;
                }
                //父节点为黑色或者父结点没有祖父结点则返回root根节点
                else if (!xp.red || (xpp = xp.parent) == null)
                    return root;
                if (xp == (xppl = xpp.left)) {//如果父结点是祖父结点是左孩子
                    if ((xppr = xpp.right) != null && xppr.red) {//叔叔结点!=null且为红色
                        //把父结点和叔叔结点变黑祖父结点变红;并把祖父结点赋值给x
                        xppr.red = false;
                        xp.red = false;
                        xpp.red = true;
                        x = xpp;
                    }
                    else {//叔叔结点==null||叔叔结点为黑色
                        if (x == xp.right) {
